...
These are principles are sometimes framed as barriers but only by full engaging with each of them will it be possible to use data to improve the quality and safety of health care. When combined we believe this will _enable_ the deployment of digital tools to solve operational and clinical problems.
...
We would recommend using the [QuickStart guide](quickstart/quickstart.qmd) to follow and 'end-to-end' walkthrough of an application from concept to implementation. This will begin with deploying the FlowEHR infrastructure for personal use with synthetic data, and end with inspecting the performance of an algorithm in the (simulated) wild.
