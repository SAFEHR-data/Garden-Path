...
They will also access a [CosmosDB](../faq/cosmosdb.qmd) database account, to store the application state. 
...
1. A FlowEHR administrator will set up the managed application repository based on 
a template repository (`-seedling`) of your choosing 
and
an `apps.{environment}.yaml` file.
It will automatically be set up with an `environment` branch. When this specific branch are pushed to, they will trigger Github workflows to deploy to their respective [environments](../faq/environments.qmd).
2. The application developer will locally clone and modify their new managed repostitory. 
3. When happy with any changes, they will open a pull request to the relevant branch, and trigger deployment. 
...
## 2. FlowEHR Admin: Add app to FlowEHR config
...
## 3. FlowEHR Admin: Deploy your app infrastructure
